...
func NewBeatReceiver(settings Settings, receiverConfig map[string]interface{}, consumer consumer.Logs, core zapcore.Core) (*Beat, error) {
	...
	reg := monitoring.Default.GetRegistry(b.Info.Beat + "-" + b.Info.ID.String())
	if reg == nil {
		reg = monitoring.Default.NewRegistry(b.Info.Beat + "-" + b.Info.ID.String())
	}
	b.Beat.MonitorRegistry = reg
